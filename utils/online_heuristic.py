...
def heur(prices_filename: str,
    ...
    # Compute the real cost and visualize the solution
    real_cost = 0
    ...
    if display:
        ...
        timestamps = VPPEnv.timestamps_headers(num_timeunits=n)
        table = list()
...
def solve_optimization_with_virtual_cost(n_timesteps: int,
    ...
    for i in range(n_timesteps):
        ...
        # Objective function
        obf = (c_grid[i] * p_grid_out[i] + c_diesel * p_diesel[i] + cvirt[i] * p_storage_in[i] - c_grid[i] * p_grid_in[
            i])
        mod.setObjective(obf)

        # Check that the model is feasible
        assert VPPEnv.optimize(mod), "The model is infeasible"
...
def solve_optimization_with_decision_vars(decision_vars: np.array,
    ...
    assert decision_vars.shape == (
        n_timesteps, 4), f"decision_vars must have shape ({n_timesteps}, 4), found {decision_vars.shape}"
...
def solve_toy_optimization_with_decision_vars(decision_vars: np.array,
                                              n_timesteps: int,
                                              shift: np.array,
                                              p_ren_pv: np.array,
                                              tot_cons: np.array,
    ...
    """
    Compute the cost of a complete solution fo the VPP problem.
    :param decision_vars: numpy.array of shape (n_timesteps * 2); the numpy array with the complete solution.
    :param n_timesteps: int; the number of timesteps in a day.
    :param shift: numpy.array; optimal demand shifts.
    :param p_ren_pv: numpy.array; the photovoltaic predictions.
    :param tot_cons: numpy.array; the total consumption predictions.
    ...
    # We follow this convention:
    # action[0] -> power sold to the grid
    # action[1] -> power generated from the diesel source

    # Check that the decision variables have length (n_timesteps, 2)
    assert decision_vars.shape == (
        n_timesteps, 2), f"decision_vars must have shape ({n_timesteps}, 2), found {decision_vars.shape}"

    grid_in = decision_vars[:, 0]
    diesel_power = decision_vars[:, 1]
    all_grid_out = []
    ...
    for i in range(n_timesteps):
        ...
        # Set the power out from the grid so that the power balance constraint is satisfied
        grid_out = tilde_cons - p_ren_pv[i] - diesel_power[i] + grid_in[i]
        all_grid_out.append(grid_out)
        ...
        # If the power balance constraint is not satisfied then the solution is not feasible
        if grid_out < 0:
            cost = np.inf
            break
        ...
        assert diesel_power[i] <= p_diesel_max
        assert grid_in[i] <= 600
        power_balance = p_ren_pv[i] + grid_out + diesel_power[i] - grid_in[i]
        assert_almost_equal(power_balance, tilde_cons, decimal=10)
...
def compute_real_cost(instance_idx: int,
    ...
    # ...or from a complete solution
        ...
        elif isinstance(decision_variables, np.ndarray):
            assert decision_variables.shape in {(n, 4), (n, 2)}, 'wrong shape for decision variables'
            decision_vars = decision_variables
        else:
            raise Exception(f"decision_variables must be a string representing a filepath or"
                            f"a numpy array of shape ({n}, 4) or ({n}, 2)")
        ...
        if np.array_equal(decision_vars, np.inf):
            ...
            real_cost, solution = \
                solve_toy_optimization_with_decision_vars(decision_vars=decision_vars,
                                                          n_timesteps=n,
                                                          shift=shift,
                                                          p_ren_pv=p_ren_pv,
                                                          tot_cons=tot_cons,
...
def create_dataframe(solution):
    ...
    timestamps = VPPEnv.timestamps_headers(num_timeunits=96)
    # table = list()
    visualization_df = pd.DataFrame(index=timestamps)
